...
class App extends Component {
  ...
  componentDidMount() {
    ...
    const weightSums = (allMatches) => {
      var sum = 0;
      for (let z = 0; z < allMatches.length; z++) {
        if (allMatches[z].group == allMatches[z].group) {
          allMatches.forEach((i) => {
            allMatches[z].sum = sum += i.totalWeight;
          });
        }
        return allMatches.sum;
        console.log("summed weight", weightSums);
      }
      for (let q = 0; q < allMatches.length; q++) {
        sum[allMatches[q].sum].push(allMatches[q]);
      }
    };
  }
